...
# supplying an additional command to go beyond the default bin width to set it at 10 units
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), binwidth = 10)

# can also set the number of bins. Best between 5 - 20 bins
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), bins = 20)

# cut the data at 500 for better visualization
...
```{r, fig.height = 3, fig.width = 4}
#lty is short for line type. visually compare 3 different aspects of our data
ggplot(USGS.flow.data) +
...
> Answer: That there is missing data in the 70s to 90s. The relationships between various categories such as gage hieght and discharge.


How can multiple options for data exploration inform our understanding of our data?

> Answer: we can better observe trends and relationships between various data.
