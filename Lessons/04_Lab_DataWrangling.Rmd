...
NTL.phys.data.1990s <- filter(NTL.phys.data, year4 %in% c(1990:1999),lakename %in% c("Tuesday Lake"))
...
NTL.phys.data.exercise3 <- 
  ...
  filter(year4 %in% c(1990:1999)) %>% mutate(month = month(sampledate)) %>% filter(month == 7)
...
summary(SurfTemp$temperature_C)
...
NTL.phys.data.exercise5 <- pivot_longer(NTL.phys.data, irradianceWater:irradianceDeck, names_to = "Sampling Location", values_to = "Irradiance")

#Exercise 6: spread temperatureC into more than one column based on the depth
NTL.phys.data.exercise6 <- pivot_wider(NTL.phys.data, names_from = depth, values_from = temperature_C)
