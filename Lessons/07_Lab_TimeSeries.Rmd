...
BRA_temp.plot <- ggplot(BRA_complete, aes(x = Date, y = Temperature_C)) + 
  geom_point() +
  geom_smooth(method=lm)
print(BRA_temp.plot)

BRA_rain.plot <- ggplot(BRA_complete, aes(x = Date, y = Rainfall )) +
  geom_point()+
  geom_smooth(method=lm)
print(BRA_rain.plot)
...
BRA_temp_ts <- ts(BRA_complete$Temperature_C, start = c(f_year,f_month),frequency=12)
BRA_rain_ts <- ts(BRA_complete$Rainfall, start = c(f_year,f_month),frequency=12)
...
BRA_temp_decomp <- stl(BRA_temp_ts, s.window = "periodic")
plot(BRA_temp_decomp)
BRA_rain_decomp <- stl(BRA_rain_ts, s.window = "periodic")
plot(BRA_rain_decomp)
...
BRA_temp_Components <- as.data.frame(BRA_temp_decomp$time.series[,1:3])

BRA_temp_Components <- mutate(BRA_temp_Components,
        Observed = BRA_complete$Temperature_C,     
        Date = BRA_complete$Date)

BRA_rain_Components <- as.data.frame(BRA_rain_decomp$time.series[,1:3])

BRA_rain_Components <- mutate(BRA_rain_Components,
        Observed = BRA_complete$Rainfall,     
        Date = BRA_complete$Date)

# Exercise 10: Visualize how the trend maps onto the data for both series
BRA_rain_components.plot <- ggplot(BRA_rain_Components)+
  geom_line(aes(y = Observed, x = Date),  size = 0.25) +
  geom_line(aes(y = trend, x = Date), color = "#c13d75ff") 
print(BRA_rain_components.plot)
...
BRA_temp_trend <- Kendall::SeasonalMannKendall(BRA_temp_ts)
summary(BRA_temp_trend)

BRA_rain_trend <- Kendall::SeasonalMannKendall(BRA_rain_ts)
summary(BRA_rain_trend)

# Exercise 12: Now remove the seasonality and apply another test. Compare the results with what you obtained on Exercise 9.

BRA_temp_trend2 <- Kendall::SeasonalMannKendall(BRA_temp_ts-BRA_temp_Components$seasonal)
summary(BRA_temp_trend2)

BRA_rain_trend2 <- Kendall::SeasonalMannKendall(BRA_rain_ts-BRA_rain_Components$seasonal)
summary(BRA_rain_trend2)
