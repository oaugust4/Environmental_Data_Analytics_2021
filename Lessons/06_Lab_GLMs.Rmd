...
> Answer: The null hypothesis is that mean of the group is is > 50.
The alternative hypothesis is that the mean is < 50.
...
PM25_Ozone.regression <- lm(data = EPAair, Ozone ~ PM2.5) 
  #geom_smooth(method = lm, se=FALSE, formula=y~x, color = "black")
summary(PM25_Ozone.regression)
...
EPAair.scatter <- ggplot(EPAair, aes(x = PM2.5, y = Ozone))+
  geom_point()+
  geom_smooth(method = "lm")
print(EPAair.scatter)
...
EPAair.corrplot <-
  ...
  na.omit()
cor_inclas <- cor(EPAair.corrplot)
corrplot(cor_inclas,method = "ellipse")
...
EPAair.AIC <- lm(data = EPAair.corrplot, PM2.5 ~ SITE_LATITUDE+SITE_LONGITUDE+Month+Year+Ozone)
step(EPAair.AIC)
...
EPAair.AIC.2 <- lm(data = EPAair.corrplot, PM2.5 ~ SITE_LONGITUDE+Month+SITE_LATITUDE+Ozone)
summary(EPAair.AIC.2)
```
